...
# [2025.12.15.]
fpga/에서 갖고 있던 최종본 파일의 코드와 module/ 에 있는 코드들을 비교하며 변경사항들을 수정했다.
대표적으로는 이전에 있던 분기 예측 실패시의 잘못된 주소 전달과 Byte Enable Logic에서의 Load시 byte-operation이 안되었던 것이 있다.

Reddit에 basic_RV32s에 대해서 포스팅을 하였다. 처음으로 공표한 셈인데, 어찌 잘 사람들이 사용해줬으면 좋겠다. 
관리자 승인까지 시간이 좀 걸려서 바로 반응을 얻지는 못했지만 그래도 의미가 크다. 
오늘은 이 basic_RV32s 프로젝트의 시작일이기 때문. 최초의 다이어그램이 12.15로부터 날짜가 찍혀있다. 하하.
fpga/에 현재 dhrystone 정식 구동 프로젝트 파일을 업로드 하였고, dhrystone-rv32i-baremetal 이라는 레포지토리를 하나 더 생성해서
Dhrystone 2.1 을 RV32I 베어메탈 환경에서 사람들이 구동할 수 있도록 만들어보았다. 
관련한 설명도 README에 추가했고, 기존에 있던 branch들 develop이나 docs를 한번 main에 merge 했다.
어찌 반응이 잘 나오길 기대해본다.

# [2025.12.16.]
Reddit에 공표도 했고, 할일들, 특히 그냥 '하면 좋은 것들'이 basic_RV32s에 조금 남아있긴 하나, 이건 다른 contributer들이 해주길 바란다. 나름의 과제랄까.?
그래서 슬슬 ima_make_rv64로 넘어갈까 했지만 ChoiCube84가 그래도 열어놓은 issue들은 해결해야하지 않겠냐라 하여, 그리하기로 했다.
열려져 있는 이슈는 3개인데, 하나는 이미 MMIO Interface를 구현하며 해결되었었다.
ChoiCube가 연 Issue인데, module/ 에 있는 fpga용 SoC를 제외한 코어의 submodule들에게는 clk_enable 신호를 제하자는 것이었다.
이는 hotfix PR에서 고쳐졌으므로 close했다. 
남은건 두가지다. 이건 좀 테크니컬한 이슈이긴하나, 지금까지 걸어온 길들로 봤을 때 그리 어려워 보이지는 않는 이슈들이다. 정말 그 당시에 한시가 급해서 미뤄둔 것 같다.

1. 같은 주소 연속 CSR 접근 (쓰기 후 읽기)시에 포워딩 경로 Commit단계 -> WB단계 구현해야한다.
2. Misaligned memory access를 misaligned store / load로 나눠서 구분하고 Exception 일으키도록 수정해야한다.

아마 이번 주 내로 끝낼 수 있을 것이라 기대한다.

오늘은 그리고 서울대학교 AI 포럼 / 채용박람회 SNU 2025 SAISF 를 다녀왔다. 
퓨리오사, 파두, 망고부스트, 소테리아, 하이퍼엑셀. 좋은 기업들을 많이 알아볼 수 있었다. 특히 소테리아가 HPC에 진심인 설계를 하는 것에 강한 끌림을 느끼기도 하고, 계열이 가장 잘 맞는 것 같다는 생각.

# [2025.12.17.]
오늘은 밤새 빌드시킨 서브 컴퓨터의 RISC-V RV32I 툴체인을 가지고 basic_RV32s 레포지토리의 fpga/ 를 테스트해보았다.
결과는 정상이었는데, 한가지 문제점이 발견되었다. 2024.2 버전에서는 2025.2 버전을 열 수가 없다는 것.
이럴수가. 기존에 RV32I46F_5SP, 46F5SP_SoC 까지만 작업했을 때는 2024.2로 했고, 이번엔 새 기분으로 2025.2로 작업했더니 이런 문제가 생겼다. 
이런 상위가 하위를 호환하지 못하는 경우에는 산업에서는 항상 최신버전이 생기면 최신으로 작업하거나, 아니면 대다수 구버전으로 하여 높은 버전에서도 열 수 있게하거나 둘 중 하나일텐데.
당장은 Issue를 해결하는게 우선이라 README에 2025.2버전으로 되었다고 설명을 추가했고, 겸사겸사 다른 장에도 이번에 새로 구현한 MMIO UART 코어랑 SoC에 대한 내용을 포함시키고 구현 환경에 대해서도 세부요소를 더했다. 
일단은 issue 해결부터 해보자. 연속 CSR 쓰기 문제부터 해결해보겠다.

# [2025.12.21.]
17일부터 Issue 두개를 모두 close 했다. 첫번째 연속 CSR 쓰기 문제는 아무래도 개발 도중에 로직들을 수정하면서 자연스레 고쳐진 것으로 확인됐다.
그간 관련 로직 변경사항이 Exception Detector랑 csr_write_enable_logic에 대한 변경이 있었는데, 같은 주소에 연속으로 접근하여도 정상적으로 읽고 쓰기가 가능하다는 것을 확인했다.
두번째로 테스트 벤치 시나리오에서 Misaligned Store와 Load에 대한 Trap Handler 분기시 문제가 있었는데, 레지스터값이 이전 값이 아니라 0으로 된다던가, 루프가 생긴다거나 하는 이슈가 있었다. 
하지만 이것도 최신 MMIO코드에 올려서 step-by-step 으로 테스팅해보니까 현상이 사라져 개발 중 해결됨으로 처리했다. 
이번에 배운 교훈은, Issue 를 생성할 때 재현가능한 당시의 코드를 같이 업로드 해야한다는 점이다. 
결론은 매우 단순했지만 그 결과까지 가기 위한 환경 설정이라 상황 재현하는 것이 가장 많은 시간을 갉아먹었다. 
Issue도 해결했고, 앞서 말한 2024.2버전의 2025파일 호환이슈를 해결했다.
2024.2 버전이 깔려있는 서브PC에서 소스파일을 동일하게 다시 Vivado 에서 프로젝트 파일을 만들어 2024.2에서도 프로젝트 파일을 정상적으로 열 수 있도록 하였다. 
develop 브랜치에 적용했고, 이제 release v2.0.0을 남겨두고 있다. 
릴리즈에서는 레포지토리의 전체 파일 하나와 (자동으로 생성됨)
소스파일만 모아둔 에센스를 같이 제공한다.
안에는 fpga/, modules/ testbenchs/ 가 제공되어 필요한 사람들이 바로 접근하여 사용할 수 있도록 하였다. 
이 Docs 브랜치도 이젠 거의 마지막이다. 이젠 RV64로 넘어갈 시기야.
그동안 정말 고생 많았고, 더 열심히 해봐야겠다. 