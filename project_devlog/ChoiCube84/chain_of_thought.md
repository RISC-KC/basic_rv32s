...
연등 시간이 시작되었다. 어제 PC Controller 수정을 완료했고, 오늘은 드디어 RV32I37F 를 만들어볼 것이다! 검증도 진행해볼 것이다.

# 2025/02/15 ~ 2025/02/16

라고 말하긴 했는데 모듈을 만들다가 14일에는 연등시간이 끝나버렸고, 15일에서 16일 동안은 집으로 주말외박을 다녀왔다.

걱정했던대로, 코딩은 그리 많이하지는 못했다. 그나마 얻었던 소득은 우리가 만든 ALU 모듈의 스케마틱이 복잡해서 못 알아보겠다는 것 정도? ... 일단 연등이 시작되었고 기초적인 RV32I37F 탑 모듈의 구조만 잡힌 상태이다.

지금부터 할 일은 다음과 같다.

1. 깜빡하고 안 만든 Data Memory 모듈을 구현한다. 만들고 나면 develop 과 feat/rv32i37f 두 브랜치에 모두 각각 병합한다.

2. 완성된 Data Memory 모듈을 RV32I37F 탑 모듈에 합쳐 완전하게 구현한 뒤, 테스트벤치를 제작하여 검증을 진행한다.

오늘은 과연 어디까지 진척시킬 수 있을까? 외박도 다녀왔으니 부지런히 코딩해봐야겠다.

안타깝게도 Data Memory 모듈의 구조만 잡는게 오늘의 성과였다. 내일은 테스트벤치를 만들어 검증을 진행하며 모듈을 완성하고, 최종적으로 RV32I37F 에 통합하여 검증을 진행하는 것이 목표이다. 오늘은 여기까지!

# 2025/02/17

사지방에 내려온 뒤 개발일지는 한 글자도 안 쓴채 구현을 시작했지만... 오늘은 유독 코딩에 집중하기 힘들었다. 외박 휴유증인지 세차 휴유증인지는 몰라도 피곤했기 때문이다. 우선 오늘은 Data Memory 모듈을 완성하고 테스트벤치를 작성하여 검증을 마무리 한 뒤 현재 RV32I37F 탑 모듈에 통합하는 과정까지는 마무리하였다. 내일이야말로 테스트벤치를 만들고 검증을 진행해보겠다. 오늘은 여기까지!

# 2025/02/18

연등 시간이 시작되었다. 테스트벤치를 만드려고 하면서 들었던 첫 번째 의문은 평소에는 입력 신호에 따른 출력값들을 확인할 수 있게 만들 수 있었는데, 탑 모듈의 경우 출력이랄게 딱히 없기 때문에 확인을 어떻게 해야할지 KHWL 에게 물어보았고, gtkwave 등을 활용하여 내부 신호의 파형을 확인하면 된다는 답변이 돌아왔다. 그래서 오늘은 우선 테스트벤치 형태를 구성하고 값을 일일히 바꿔주는 대신 클럭 시간만 흐르게 하고, dumpfile dumpvars 등의 명령어를 활용하여 vcd 파일을 만들어보려고 하였다. 그 과정에서 탑 모듈에서 신호가 잘못 전달된 내용 등을 모두 수정하였다.

일단 연등이 종료된 지금 시점까지도 신호 파형을 확인할 수 있는 vcd 파일을 만드는 방법을 알아내지 못했다. 내일 이어서 해봐야겠다. 오늘은 여기까지!

# 2025/02/19

오늘은 연등을 시작하고 우선 vcd 파일을 생성하는 방법에 대해 알아봤다. dumpfile 명령어에서 디렉토리 부분을 이리저리 수정하니 vcd 파일이 만들어졌다.

그렇게 만들어진 vcd 파일의 뷰어를 열심히 찾아다녔고, VCDrom 을 최종적으로 이용하게 되었다.

그리고 그 VCDrom 으로 waveform 의 파형을 확인했는데, 아무것도 안나왔다... 그래서 아무 모듈이나 급하게 파형을 확인해 봤는데, (ALU Control 모듈이었다.) 그건 또 제대로 나왔다. 파형을 표시해주는 VCDrom 에 문제가 있는 것이 아니라 내가 뭔가 코딩을 잘못했다는 걸 알 수 있었다.

다시 한 번 파형을 자세히 살펴보니, 대부분의 신호가 전부 X 였다. X는 어떤 신호일지 알 수 없는 경우에 뜨는 걸로 알고있는데, 혹시 reset 신호를 이용하여 초기화를 진행하지 않아서 그런 것인지 의심이 되었다. reset 신호로 1을 인가하고 잠시 시간이 흐른 후에 다시 0으로 바꾸어주니, Instruction Memory 모듈에 하드코딩 해두었던 첫 번째 명령어는 제대로 수행하는 것을 확인했다! 그러나 다음 두 개의 명령어를 제대로 실행하지 않아 뭐가 문제인지 살펴보는데... 깜빡하고 PC Controller 모듈을 탑 모듈에 포함을 안 시켰던 것이다! 포함시켜 주고난 뒤 파형을 확인해봤고, 그 결과는... 성공적이었다!

**드디어 처음으로 제대로 작동하는 RV32I Top module 구현에 성공한 것이다!**

아직 더 검증할 명령어들도 많고, 모든 명령어를 지원하지는 않지만, 아마 오늘의 말도 안되는 전율은 정말 간만에 느껴보는 그것이었다.

그 흥분에 취한 사이, 연등 시간이 끝났고, 추가적인 검증은 내일 이어가기로 했다. 오늘은 여기까지!

# 2025/02/20

오늘은 RV32I37F 탑 모듈 검증을 계속 해볼 것이다. 지금은 Instruction Memory 에 명령어들을 하드코딩 해둔 상태인데, readmemh 등의 명령어를 이용하여 이를 고쳐볼까 생각하고 있다. 그리고 KHWL 이 보내준 RV32I 검증 테스트벤치 파일을 우리 모듈에 맞게 수정해볼 생각이다.

# 2025/02/21

라고 했는데 테스트벤치 파일이 안 보여서 일단 직접 테스트벤치를 수정해보기로 했다. 우선 피보나치 수열을 각 메모리의 인덱스에 저장해보도록 테스트를 해보고 싶었고, Instruction Memory 의 테스트벤치에서 해당 코드들을 만들어보고 test_code.mem 파일에 저장할 수 있게 했다. 그리고 그걸 RV32I37F 탑 모듈에서 이용하도록 하였다.

# 2025/02/22

오늘은 RV32I37F 의 37가지 명령어를 모두 테스트해보기 위해 GPT 를 이용하여 37개의 명령어가 담긴 테스트벤치를 만들었다.

그런데, 만든 탑 모듈 파형을 VCDrom 으로 확인해봤을 때, 중간에 잘리고 뒷 부분 명령어가 어떻게 처리되었는지 확인할 수 없었다. 사이트의 결함이라고 생각했던 나는 별별 방법을 다 시도해보았다. GTKwave 를 실행하는데, 이미지 파일로 결과를 내보내도록 하려 한다거나, python 을 이용하여 matplotlib 과 vcdvcd 라이브러리로 파형이미지를 확인하려 한다던가... GTKwave 쪽은 이미지 생성조차 되지 않았고, python 쪽은 이미지가 생성되긴 했는데 역시 중간에 뚝 끊겼다.

그러다 혹시 RV32I37F 테스트벤치에서 러닝 타임이 너무 짧은게 아닌가 싶어 늘려보니, 뒷부분이 충분히 커버되는 것을 확인할 수 있었다! 그러나 여전히 문제는 있었다. Data Memory 에서 읽고 쓰는 부분에서 신호가 x 표시가 되어있던 것이다. 이건 값이 어떤 값일지 예측이 안된다는 의미였고, 결국 Data Memory 에서 메모리의 값들을 모두 0으로 초기화하도록 수정하였다. 그 결과 x 표시는 사라졌다.

내일은 본격적으로 테스트벤치의 명령어들의 순서, 인자 값들을 변경해가며 동작을 제대로 검증할 수 있는 테스트벤치를 완성해볼 계획이다. 오늘은 여기까지!

# 2025/02/23

교회를 다녀오고 나서 잠깐 쉬다가 바로 사지방에 내려왔다. 그리고 4시간 정도를 갈아서 명령어 순서를 재배치 했다. 우선 I 타입 ALU 연산들을 사용하여 몇몇 레지스터의 값을 초기화 하고, 그 값들을 기반으로 다른 명령어들을 수행토록 하였다. 

그런데 문제가 생겼다. 파형을 확인해보니, 00000ABC 일것이라고 생각했던 값이 FFFFFABC 로 나왔던 것이다! 생각해보니 12비트짜리 imm 값을 'sign extension' 을 하는데, 12'ABC 는 MSB 가 1이였던 것이다. 그렇게 연등 시간동안 다시 시간을 갈아넣어서 수정을 완료했다.

노가다를 하면서 KHWL 와 CSR File 모듈 구현과 관련하여 이야기를 했는데, Register File 모듈 처럼 읽기는 비동기로 진행하고, 쓰기만 clk 신호에 맞춰 동기식으로 구현하기로 했다. 이 때, 어떤 값이 들어와야 하는지는 ALU 쪽에서 계산을 해주기 때문에 굳이 3비트짜리 csr_op 신호를 쓸 것도 없이 1비트짜리 csr_write 로 대체해도 된다는 결론에 도달했다. 자꾸만 할 일이 증식하고 있다. 큰일났다...

이제 연등시간이 끝났다. 내일은 만들어둔 테스트벤치의 파형을 보면서 동작 검증을 해봐야겠다. 눈이 좀 아플 것 같다. 오늘은 여기까지!

# 2025/02/24

오늘은 근무가 개인정비시간에 있어서 점호가 끝난 뒤 샤워를 하고 나서야 사지방에 와서 개발을 시작했다. 어제 테스트벤치를 완성했으니 이제 신호들을 검증하는 단계에 착수했다.

일단 대부분의 연산은 ALU 를 사용하기 때문에 ALU 의 결과값 신호를 먼저 확인하고 있었다. 
그런데 이상한 부분이 있었다. BC000000 값을 4만큼 Shift Right Arithmetic 연산을 적용했을 때 FBC00000 가 나와야 하는데 그냥 FFFFFFFF 가 나와버린 것이다! 
뭔가 이상하다 싶어 근처의 src_B 값을 보니 00000004가 아니라 00000404 가 찍혀있었다!

문제가 되었던 부분은 imm 신호 부분이었다. imm 신호의 [4:0] 부분을 잘라내어 shift 할 값으로 설정했어야 하는데, 그냥 통째로 imm 값을 주는 바람에 엉뚱한 값만큼 shift 연산을 해버린 것이었다! KHWL 과 이 문제에 대해 상담한 결과 ALU 쪽에서 인덱스를 슬라이싱해서 계산을 하도록 수정하기로 했다가, R 타입 sra 명령어에서는 슬라이싱이 이루어지면 안된다는 것을 발견하고 다시 논의가 시작되었다. 내가 낸 의견은 src_B 를 위한 MUX 를 shamt 값을 포함하도록 확장하는 것이었다. 그리고 KHWL 이 최종적으로 동의했다. 그러나 연등 시간이 끝나버렸고 내일 이어서 하기로 했다. 오늘은 여기까지!

# 2025/02/25

오늘은 평출을 다녀온 뒤 샤워를 하고 조금 늦게 사지방에 왔다. 제일 먼저 할 일은 shamt 신호를 ALU 의 src_B MUX에 추가해주는 것이었다.

이 부분을 구현하는 것에 대해서 KHWL 에게 물어봤는데, 2비트 신호를 3비트 신호로 확장하려 하고 있었다. 근데 어짜피 4개 중에 고를건데 굳이 3비트로 할 필요가 있는지 물었고, KHWL 에게 MUX의 비활성화 신호를 주어야 하기 때문에 3비트로 늘려야 한다는 답변이 돌아왔다. 나는 MUX 에서 특정 신호를 계속 보낸다 하더라도 목적지 모듈에 비활성화 신호가 가는 이상 해당 값이 사용되는 문제는 없다고 주장했고 KHWL 으로부터 그걸 보장할 수 있느냐는 답변이 돌아왔다. 논의는 우선 여기서 중단했다. 개발 일정이 조금 빠듯했기 때문에 일단 3비트인 상태로 구현하고 나중에 논의를 이어가기로 했다. 

# 2025/02/26

그렇게 srai 명령어 관련 이슈를 해결하고 난 뒤, 계속해서 탑 모듈 테스트벤치의 파형을 확인하고 있었다. 이번에는 AUIPC 부분에서 제대로 덧셈 연산이 수행되지 않는다는 것을 발견했다. JAL 명령어 부분에서도 같은 문제가 발생했는데, 이는 PC Controller 모듈에서 해당 명령어에 대해 ALU 에 연산을 수행하도록 신호를 제대로 보내지 않고 있었기 때문이었다. 이 문제를 해결하고 다시 검증을 진행한 결과, ALU 의 alu_result 신호는 모두 올바른 값을 내보내는 것을 확인할 수 있었다.

아직 검증할게 산더미같이 남았지만, 문제점을 제대로 수정할 수 있어 다행이었다. 나머지 ALU 신호들과 다른 모듈들 검증은 내일 이어서 해야겠다. 오늘은 여기까지!

# 2025/02/27 ~ 2025/03/02

한동안 검증에 매달리느라 일지를 작성하지 못했다. 그간의 성과를 지금이라도 담아보려고 한다.

1. 기존에는 VCDrom 을 이용해왔는데 surfer 라는 괜찮은 대안을 발견했고 계속 그걸 이용하여 검증을 진행하고 있다. 

2. Instruction Decoder 에서 나오는 raw_imm 신호를 20비트짜리로 바꿨다. 언젠가 하겠다고 내버려뒀었는데, 더 늦기전에 적용을 해뒀다. 다행히 작동을 정상적으로 이루어지는 것 같다.

3. Load 명령어와 관련하여 심각한 이슈 한가지를 발견했다. 현재 우리 탑 모듈에서는 clk 상승 신호에 맞추어 모든 모듈이 한 번에 작동하는 방식인데, Control Unit 에서 내보내는 read 신호가 clk 상승 신호에 맞추어 0에서 1로 변하는 경우, Data Memory 모듈에서 read 신호를 1로 읽어 제대로 읽기 동작을 수행해야 하지만 0으로 인식해버리는 바람에 읽기 동작이 제대로 일어나지 않는 문제점이 있었다. 오랫동안 이 문제에 대해 KHWL 과 의논했고, 결론적으로 Data Memory 에서 읽기 동작이 제대로 수행되었는지 확인하는 값을 내보내고, 이를 PC Controller 에서 받아 Load 명령어가 완전히 수행된 이후에 다음 명령어로 넘어갈 수 있게끔 설계를 변경하기로 했다.

여기까지가 오늘 연등전까지 진행된 내용이다. 지금 이 글을 쓰고 있는 시점은 19:48 이라 아직 더 개발할 시간이 남아있긴 하지만 8시에 다시 근무 투입이기 때문에 기록을 남길만큼 충분히 진전을 만들 수 있을지는 모르겠다. 일단 남은 시간동안 계속 만들어봐야겠다.

# 2025/03/03

그렇게 근무 전 쉬는 시간, 오늘 오전과 오후, 그리고 현재 연등시간까지 진척된 내용들은 다음과 같다.

1. Data Memory 에서 Control Unit 으로 보내는 read_done 신호, Control Unit 에서 PC Controller 로 보내는 pc_stall 신호를 추가하여 읽기 동작이 수행이 완전히 완료되어야만 다음 명령어로 넘어갈 수 있도록 수정하였다.
...
그렇게 오랜 시간동안 일지를 쓰지 못한채 많은 일들이 있었다.

원래대로라면 내가 RV32I43F 탑 모듈을 구현해야 했지만, KHWL 에게 직접 탑 모듈을 구현해보는 것이 어떨지 제안했고, KHWL이 이를 승낙했다. 나는 그 동안 캐시를 만들어보기로 하였다.

그렇게 KHWL은 내가 만든 RV32I37F 탑 모듈을 기반으로 CSR File 이 추가된 RV32I43F 탑 모듈을 구현하는데 성공했고, 얼마 지나지 않아 훈련 기간이 시작되었다. 나는 그 동안 휴가를 나가 Instruction Cache 를 만들어 보았는데, 물론 예상대로 잘 되진 않았고 14일 오늘 복귀해서야 완성할 수 있었다.

내일은 Data Cache 를 만들어볼 것이다. 오늘은 여기까지!

# 2025/03/15 ~ 2025/03/17

그렇게 Data Cache 를 만들기 시작했다. 그러다가, Load 명령어들을 한 사이클 안에 끝낼 방법에 대해 이야기가 나와 그 부분을 먼저 알아보기로 했다. KHWL 이 여러 레퍼런스를 찾아보다가 어떤 데이터 메모리 모듈 코드를 보내줬고, 분석해본 결과 우리의 것과 거의 동일한 데이터 메모리였다.

그러나 해당 모듈을 대신 넣어 우리의 탑 모듈을 시험해본 결과 한 사이클 안에 데이터 메모리에서 읽어오는게 가능했고, 우리 모듈과의 유일한 차이점이었던 read_data 의 assign 방식을 해당 레퍼런스처럼 combinational 하게 바꿔보기로 했다. 그러자 제대로 동작했고, 거기서부터 대공사가 시작되었다.

우선 Data Memory 에서 바로 읽는 것이 가능해지면서 read_done 신호가 필요없어지게 되었고, Control Unit 에서도 필요없어지게 되었다. 그렇게 만들고 보니, pc_stall 신호가 read_done 신호에 의해 만들어졌기 때문에, pc_stall 신호도 필요없어졌다고 판단하여 지우려고 했는데, KHWL의 제안으로 우선 신호를 유지하기로 했다. 이 과정에서 pc_stall 신호가 write_done 신호를 이용하여 대신 만들어지게 되었고, 이에 맞추어 pc_controller 에서 write_done 신호를 받는 대신 pc_stall 신호로만 pc 값 업데이트를 중단하도록 하였다.

이런 식으로 모듈들을 수정하고 나서, 탑 모듈 수정을 시작했고 RV32I37F 모듈의 경우 읽기가 지연되지 않고 한 사이클만에 바로 실행되는 것을 확인할 수 있었다. RV32I43F 는 KHWL이 추후에 검증하기로 했다.

내일부터는 다시 Data Cache 를 만들어볼 것이다. 오늘은 여기까지!

# 2025/03/18 ~ 2025/03/24

그리고 결국 24일이 되어서야 데이터 캐시를 완성할 수 있었다.

만든 캐시는 2-way set associative 방식이고, LRU 방식으로 작동한다.

읽기 작업의 경우 cache hit 가 발생하면 바로 읽어오고 실패할 경우 두 라인중 invalid 한 라인이 있으면 Data Memory 에서 해당 값을 불러와 해당 라인에 저장하는데, 지금은 물리적 지연을 고려하지 않고 설계하였고, update -> read 순으로 총 2사이클이 필요하다. invalid 한 라인이 없으면, 현재 밀려나야 하는 라인을 담은 lru 변수를 보고 선택된 라인을 업데이트 하는데, 이 때 원래 기존의 값이 업데이트 되어있는 경우에는 flush 를 먼저 진행한다. 따라서 flush -> update -> read 순으로 3사이클이 걸린다. 업데이트 된 바가 없다면 당연히 update -> read 로 2사이클만 걸린다.

쓰기 작업의 경우도 비슷했다. cache hit 가 발생하면 바로 마스크에 맞추어 쓰기를 진행하고, 업데이트가 이루어졌음을 저장한다. 실패할 경우 invalid 한 라인이 있으면 해당 라인에 쓰기를 진행하고, 업데이트가 이루어졌음을 저장한다. 여기서... 라고 쓰자마자 문제가 될만한 부분을 알아냈다...

그리고 시간이 너무 늦어서 해당 문제는 내일 정리하고 구현을 마무리 하려고 한다. 오늘은 여기까지!

# 2025/03/25

어제 찾아냈던 문제가 된 부분은 쓰기 작업에서의 문제점이었다.

현재 구현된 Data Cache 는 line 에 없는 tag 의 값을 write 하려고 하면, Data Memory 에서 기존의 값을 불러오지 않고 입력으로 들어오는 쓰고자 하는 데이터만 들어온다. 이 과정에서 문제가 발생할 수 있는데, 예를 들어 원래 Data Memory 에 0xDEADBEEF 가 적혀있다고 가정하자. 사용자는 최상위 바이트를 0x12 로 수정하고 싶어서 write_data 를 0x12000000으로, write_mask 를 0b1000 으로 주었다. 현재 캐시에서는 나중에 해당 주소가 flush 될 때 의도한 값인 0x12ADBEEF 가 아닌 0x12000000 을 돌려주게 되는 것이다!

이 문제를 해결하기 위해서는 각 line 이 flush 될 때 현재 업데이트된 부분의 마스크를 함께 전달하는 것이다. 그래서 각 line 별 현재 마스크를 저장할 내부 변수와 Data Cache 에서 Data Memory 로 전달되는 flush_mask 신호를 새롭게 추가하여 구현을 완성할 것이다.

# 2025/03/26 ~ 2025/05/06

그로부터 일지를 쓰지 않은지 많은 시간이 흘렀다. 타이밍을 고려하지 않고 캐시 구현을 완료했다가, 필요한 캐시 스펙들이 구체화되고, 이에 맞춰 수정을 하려다 구현이 어려워 지체가 심하게 이루어지고, 마침 발목 수술로 병원을 입원하면서 진척이 늦어지고 하다가, 오늘 캐시를 구현하고 검증을 완료했다.

새로운 캐시는 한 라인에 32 'bit' 가 아닌 32 'byte' 를 담고 있고, 기존과 마찬가지로 2-way set associative 로 구현되었다. 또한 용량을 확장하여 64Kib 의 용량을 가지게 되었다.

그런데 검증 과정중에 조금 신경쓰이는 부분이 있었다. 한 set 의 두 line 이 모두 꽉차서 기존의 값을 밀어내고 새로운 값을 Data Memory 에서 읽어와야 하는 경우에, 값을 불러오는 클럭에서는 hit 가 0이여야 할 것 같은데, 1이 나온 것이다. 동작에는 이상이 없어 우선 넘어가긴 했는데, 내일 이 부분을 조금 고민해봐야 할 것 같다.

# 2025/05/07

이 문제를 KHWL 과 논의한 결과, 지금 구현한 캐시에 hit 로 신호가 나오는 것은 당연하지만, 그래서는 안된다는 결론에 도달했다. 다시 말해, 캐시를 다시 만들어야 하는 것이다...

일단 그것과는 별개로 이제 RV32 에서 RV64 로 비트를 확장해야 하기 때문에 ima_make_rv64 라는 이름의 레포지토리를 새로 만들었다. 이 레포지토리는 RV64'IMA' 를 구현하는 것을 목표로 하는데, i'ma 라는 표현이 I'm gonna 라는 표현의 축약임을 이용한 언어 유희인 것이다.

일단 캐시를 다시 만들어야 할지, 64비트 확장을 먼저해야 할지 고민해봐야겠다. 지금으로서는 캐시를 다시 만든다는 것은 FSM 을 이용하여 처음부터 다시 설계를 해야한다는 것을 의미하는데...

KHWL 과 논의한 결과, 우선 64비트 확장 먼저 진행하고, IMA 확장이 구현된 다음에 캐시를 건드리기로 했다. 역시 그 편이 속도도 더 잘 날 것 같다. 한동안 캐시에 너무 시달려서, 코드를 건드리려 하면 많이 힘들기 때문이다. 좀 잊어뒀다가 나중에 코딩하는게 편할 것 같다.

이제 연등 시간이 슬슬 마무리 되어간다. 내일 이어서 해야겠다. 오늘은 여기까지!

# 2025/08/05

64비트 확장을 개시한 뒤로는 좀처럼 이 레포지토리에서는 작업하지 않게 되었다. 현재 RV64I 를 우선적으로 구현하는데 집중하고 있다. 캐시 구현은 이후가 될 예정이다. 

추가적인 basic_rv32s 레포지토리 진척 사항이 생기면 계속 기록을 이어나가겠다. 오늘은 여기까지!
